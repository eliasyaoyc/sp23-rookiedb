...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record record : records) {
            ...
            DataBox value = record.getValue(index);
            int partitionNumber = HashFunc.hashDataBox(value, pass) % partitions.length;
            partitionNumber = partitionNumber < 0 ? partitionNumber + partitions.length : partitionNumber;
            partitions[partitionNumber].add(record);
        }
        return;
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        // Building Stage
        Map<DataBox, List<Record>> hashTable = new HashMap<DataBox,List<Record>>();
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() <= limit || rightPartitions[i].getNumPages() <= limit) {
                this.buildAndProbe(leftPartitions[i], rightPartitions[i]);
            } else {
                this.run(leftRecords, rightRecords, pass + 1);
            }
        }
    }

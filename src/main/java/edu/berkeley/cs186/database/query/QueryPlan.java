...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minScanCost = minOp.estimateIOCost();
        List<Integer> eligibleIndexColumns = getEligibleIndexColumns(table);
        int minIndex = -1;
        for (int i = 0; i < eligibleIndexColumns.size(); i++) {
            IndexScanOperator indexScanOp = new IndexScanOperator(transaction, table, selectPredicates.get(i).column,
                    selectPredicates.get(i).operator,
                    selectPredicates.get(i).value);
            int indexScanCost = indexScanOp.estimateIOCost();

            if (indexScanCost < minScanCost) {
                minScanCost = indexScanCost;
                minOp = indexScanOp;
                minIndex = i;
            }
        }
